...
def loss_recovered(
    ...
    with model.trace(text, **tracer_args, invoker_args=invoker_args):
        if io == 'in':
            x = submodule.input
            if normalize_batch:
                scale = (dictionary.activation_dim ** 0.5) / \
                    x.norm(dim=-1).mean()
                x = x * scale
        elif io == 'out':
            x = submodule.output
    ...
    assert len(
        x.shape) == 3, f"Expected x to have shape (B, L, D), got {x.shape}, output_is_tuple: {output_is_tuple}"
...
def evaluate(
    ...
    device="cpu",
    n_batches: int = 1,
    return_loss_recovered: bool = True,
):
    assert n_batches > 0
    out = defaultdict(float)
    active_features = t.zeros(dictionary.dict_size,
    ...
    for _ in tqdm(range(n_batches), desc="Evaluating"):
        ...
        # l2 ratio
        l2_ratio = (t.linalg.norm(x_hat, dim=-1) /
                    t.linalg.norm(x, dim=-1)).mean()
        ...
        # Skip loss_recovered calculation if activations is not from a buffer
        # This is because loss_recovered requires access to the original model and text,
        # which are only available when using ActivationBuffer or NNsightActivationBuffer
        if not isinstance(activations, (ActivationBuffer, NNsightActivationBuffer)) or not return_loss_recovered:
            continue
        ...
        frac_recovered = (loss_reconstructed - loss_zero) / \
